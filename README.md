...
详细的功能介绍与操作技巧，请参阅 [**项目深度文档 (PROJECT_DETAILS.md)**](./PROJECT_DETAILS.md)。
...
- **🛠️ 丰富的编辑工具**：
  - 支持插入矩形、圆形、文本框等基础图形。
  - 支持上传并嵌入图片。
  - **多媒体嵌入**：支持嵌入 YouTube/Bilibili 视频及本地/网络 MP4 视频，内置全屏播放与播放器控制。
  - 自由连接线：可在任意节点或元素间建立逻辑关联。
- **📤 多格式导出**：
  - **高清图片**：支持导出 PNG (透明背景) 和 JPG (白色背景) 格式。
  - **结构化文档**：支持导出 Markdown (.md) 和 纯文本 (.txt) 格式。
  - **专属格式**：支持导出加密的 `.amf` 文件，确保数据安全。
